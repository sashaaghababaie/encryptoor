...
import { LuExternalLink, LuEye, LuEyeOff, LuTrash } from "react-icons/lu";
    ...
    >
      ...
      >
        <motion.ul className="flex flex-col gap-4 mt-2">
          {data.map((item, index) => (
            <ItemView key={index} index={index} setData={setData} {...item} />
          ))}
        </motion.ul>
      </div>
    </motion.div>
...
const LoginView = ({ setData, ...item }) => {
  ...
  const [timer, setHoldProgress] = useState(0);
  //   const [deleteThis, setDeleteThis] = useState(false);
  const [hoverTrash, setHoverTrash] = useState(false);
  const [hoverEdit, setHoverEdit] = useState(false);

  const deleteItem = () =>
    setData((prev) => prev.filter((d) => d.title !== item.title));
  ...
  const [showed, setShowd] = useState(false);
  useEffect(() => setShowd(true), []);
  return (
    <motion.li
    ...
    >
      ...
      <div className="flex w-full group">
        ...
        <div className="hidden group-hover:flex absolute top-1 right-1 gap-1 items-center">
          ...
          >
            {hoverTrash ? (
              <span className="block text-xs font-bold w-[126px] shrink-0">
                Hold to delete
              </span>
            ) : (
              <LuTrash />
            )}
            {/* <span className="w-[126px] flex shrink-0 gap-1 items-center justify-start">
              hold
              <LuTrash />
            </span> */}
          </motion.button>
          <button className="w-[52px] h-[52px] rounded-full bg-blue-600/90"></button>
        </div>
      </div>
      <AnimatePresence>
        {isOpen && (
